...
unsigned int nStakeMinAge = 60 * 60 * 3; // 2 hours
...
const string strMessageMagic = "cydercoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 10) // CYDER
    {
        nSubsidy = 8000000 * COIN;
    }
    else if(pindexBest->nHeight < 700000) // 0 rewards
    {
        nSubsidy = 0 * COIN;
    }
...
// cydercoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "cydercoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "CYDER born today";
        CTransaction txNew;
        txNew.nTime = 1501091804;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x8c1927641692f5b17e829222728b1341a783a0651651291f13f9c120e51ab887"));
        block.print();
